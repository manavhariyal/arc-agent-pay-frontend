...
export default function Dashboard() {
  ...
  const userBalanceAmount = userBalance ? parseFloat(userBalance.balance.toString()) : 0;
  const isBalanceLow = userBalanceAmount < 10 && userBalanceAmount > 0;
  const isBalanceEmpty = userBalanceAmount === 0;
  ...
  return (
    <AppLayout>
      <div className="space-y-8">
        ...
        {isConnected && isOnArcTestnet && isBalanceLow && (
          <motion.div
            ...
            className="rounded-2xl border border-amber-500/30 bg-amber-500/8 p-4 flex items-center justify-between gap-4"
          >
            <div className="flex items-center gap-3">
              <AlertTriangle className="w-5 h-5 text-amber-400 shrink-0" />
              <div>
                <p className="text-sm font-bold text-amber-300">Low auto-payment balance!</p>
                <p className="text-xs text-amber-400/70">You only have {userBalanceAmount.toFixed(4)} USDC left. Deposit more to keep payments running.</p>
              </div>
            </div>
            <Button
              onClick={() => setDepositOpen(true)}
        ...
        {isConnected && isOnArcTestnet && isBalanceEmpty && agents.length > 0 && (
          <motion.div
            initial={{ opacity: 0, y: -8 }}
            animate={{ opacity: 1, y: 0 }}
            className="rounded-2xl border border-rose-500/30 bg-rose-500/8 p-4 flex items-center justify-between gap-4"
          >
            <div className="flex items-center gap-3">
            ...
            <Button
              onClick={() => setDepositOpen(true)}
              size="sm"
              className="h-8 px-3 text-xs bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 rounded-lg shrink-0"
            >
              Deposit Now
            </Button>
          </motion.div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-bold text-amber-300 mb-1">
                  {alerts.length === 1 ? "1 agent has a low balance" : `${alerts.length} agents have low balances`}
                </div>
                  ...
                  {alerts.map((alert) => (
                    <Link key={alert.agentId} href={`/agents/${alert.agentId}`}>
                      <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20 hover:border-amber-500/40 transition-all cursor-pointer">
                        ...
                        <span className="text-[10px] font-mono text-amber-400/70">{alert.balance.toFixed(4)} USDC</span>
                      </div>
                    </Link>
                  ))}
                </div>
        ...
        {isConnected && !isOnArcTestnet && (
          ...
          >
            ...
            <span className="text-sm font-medium flex-1">Switch to Arc Testnet (Chain ID: {ARC_NETWORK.chainId}) to use the dashboard.</span>
        ...
        {!isConnected && (
          ...
          >
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <p className="text-white/40 text-sm">Link MetaMask to Arc Testnet to view your balance, register agents, and send USDC payments on-chain.</p>
            </div>
            <WalletButton />
          </motion.div>
        )}
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-4 gap-5">
          ...
          <motion.div variants={item} whileHover={{ y: -2 }} transition={{ duration: 0.2 }}>
            <div className="glass-panel-elevated p-6 rounded-2xl h-full">
              ...
              <div className="text-3xl font-black text-white leading-none">
                {totalVolume > 0 ? <span>{formatUSDC(totalVolume)}</span> : <span className="text-white/20">0.00 USDC</span>}
              </div>
              <div className="mt-2 text-xs text-purple-400/60">{confirmedTxs.length + backendConfirmed.length} confirmed transactions</div>
            </div>
          </motion.div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-7">
                  ...
                  {agents.slice(0, 3).map((agent) => (
                    <Link key={agent.id} href={`/agents/${agent.id}`}>
                      <div className="p-4 hover:bg-white/[0.02] transition-colors flex items-center justify-between cursor-pointer">
                        <div className="flex items-center gap-3">
                          <div className={`w-1 h-10 rounded-full shrink-0 ${agent.status === "active" ? "bg-emerald-400" : agent.status === "idle" ? "bg-sky-400" : "bg-amber-400"}`} />
                          <div>
                            <div className="font-semibold text-sm text-white flex items-center gap-2">
                              {agent.name}
                              {agent.status === "active" && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />}
                            </div>
                            <div className="text-[11px] text-white/35 mt-0.5 font-mono">{truncateAddress(agent.walletAddress)}</div>
                          </div>
                        </div>
                        <Badge variant="outline" className={`capitalize text-[10px] ${agent.status === "active" ? "text-emerald-400 border-emerald-400/20 bg-emerald-400/10" : agent.status === "idle" ? "text-sky-400 border-sky-400/20 bg-sky-400/10" : "text-amber-400 border-amber-400/20 bg-amber-400/10"}`}>
                          {agent.status}
                        </Badge>
                      </div>
                    </Link>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
            ...
            <div className="glass-panel rounded-xl overflow-hidden min-h-[160px]">
              {backendTxs.length === 0 && transactions.length === 0 ? (
                <div className="p-8 text-center">
                  <div className="w-10 h-10 mx-auto mb-3 rounded-xl bg-white/[0.04] flex items-center justify-center">
                ...
                <div className="divide-y divide-white/[0.04]">
                  {backendTxs.slice(0, 4).map((tx) => (
                    <div key={tx.id} className="p-4 hover:bg-white/[0.02] transition-colors flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full shrink-0 ${tx.status === "success" ? "bg-emerald-400" : tx.status === "pending" ? "bg-amber-400 animate-pulse" : "bg-rose-400"}`} />
                        <div>
                          <div className="font-medium text-sm text-white">{tx.agents?.name ?? "Scheduled"}</div>
                      ...
                      <div className="text-right">
                        <div className="font-mono text-sm font-semibold text-white">-{tx.amount} USDC</div>
                        <Badge variant="outline" className={`mt-1 text-[10px] capitalize ${tx.status === "success" ? "text-emerald-400 border-emerald-400/20 bg-emerald-400/10" : tx.status === "pending" ? "text-amber-400 border-amber-400/20 bg-amber-400/10" : "text-rose-400 border-rose-400/20 bg-rose-400/10"}`}>
                          {tx.status}
                        </Badge>
                      </div>
                    </div>
      ...
      <DepositDialog open={depositOpen} onClose={() => { setDepositOpen(false); fetchBalance(); }} />
    </AppLayout>
  );
}
