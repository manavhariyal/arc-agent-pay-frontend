...
export default function Analytics() {
  ...
  const COLORS = ["#0A84FF", "#22F0FF", "#8b5cf6", "#10b981", "#f59e0b"];
        ...
        <div className="space-y-6 max-w-7xl mx-auto">
          <div>
            <h1 className="text-4xl font-black tracking-tight text-white mb-2 flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-[#05D8EA]/20 flex items-center justify-center border border-[#05D8EA]/20">
                <BarChart3 className="w-7 h-7 text-[#22F0FF]" />
              </div>
              Analytics
            </h1>
            <div className="h-1 w-16 bg-[#22F0FF] rounded-full mt-2"></div>
          </div>
          <div className="glass-panel-elevated rounded-2xl p-12 text-center">
            <Wallet className="w-12 h-12 mx-auto mb-4 text-[#3AB4FF]/30" />
    ...
    <AppLayout>
      <div className="space-y-8 max-w-7xl mx-auto">
        <div>
          ...
          <div className="h-1 w-16 bg-[#22F0FF] rounded-full mb-4 mt-2"></div>
        ...
        {totalTxCount === 0 ? (
          <div className="glass-panel-elevated rounded-2xl p-12 text-center">
            <Send className="w-10 h-10 mx-auto mb-4 text-[#3AB4FF]/30" />
                ...
                <div className="flex-1 min-h-0 p-4 pt-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData} margin={{ top: 0, right: 8, left: -20, bottom: 0 }}>
                      ...
                      <RechartsTooltip
                        contentStyle={{ backgroundColor: "rgba(10,13,25,0.95)", borderColor: "rgba(10,132,255,0.3)", color: "#fff", borderRadius: "12px", fontSize: 12 }}
                        formatter={(value: number) => [formatUSDC(value), "Volume"]}
                      />
                      <Area type="monotone" dataKey="amount" stroke="#0A84FF" strokeWidth={3} fillOpacity={1} fill="url(#colorVol)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </Card>
                ...
                <div className="flex-1 min-h-0 p-4 pt-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} margin={{ top: 0, right: 8, left: -20, bottom: 0 }}>
                      ...
                      <RechartsTooltip
                        contentStyle={{ backgroundColor: "rgba(10,13,25,0.95)", borderColor: "rgba(34,240,255,0.3)", color: "#fff", borderRadius: "12px", fontSize: 12 }}
                        cursor={{ fill: "rgba(255,255,255,0.03)" }}
                        formatter={(value: number) => [value, "Transactions"]}
                      />
                      <Bar dataKey="count" radius={[6, 6, 0, 0]} maxBarSize={40}>
                        {chartData.map((_, index) => (
                          <Cell key={index} fill={index === chartData.length - 1 ? "#22F0FF" : "#0A84FF"} opacity={0.8} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
